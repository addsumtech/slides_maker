...
- Output still lands in `~/Downloads/<deck>/` (the normal rule); the assembled `.pptx`
  is the deliverable, with `<deck>.pdf` beside it, and a `render/` of the final PNGs +
  `viewer.html`. Section subagents' self-renders each drop a `section_NN_*.pdf` beside their
