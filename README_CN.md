...
- **讲稿一定给，动画你说了算。** 演讲型 deck 逐页备注里有完整讲稿，拿到手就能直接开讲。点击渐显——一拍一拍地放出内容——是面试里问你的，不是默认塞给你的：你说要纯翻页，交付的就是静态 deck，那是**正确结果，不是少了功能**。真要动画时，它自己判断哪几页值得，并且把那几页**整页排完**，而不是动一半留一半。
...
1. **问清楚。** 给谁看、讲多久、现场讲还是发出去自读、要什么风格。你用短句回答就行，没想好就说「你定」——它会自己选出稳妥的答案并**在动手前先贴出来**，选错了你扫一眼就能否决，不用等一版重建（只有主题、素材这类只有你知道的信息仍然要问）。
2. **读材料（或联网调研）。** 论文、文档、代码库——还有 Word / Excel / CSV / 图片 / 视频 / 音频，以及按目的取舍的整本书——都如实读取，一律先精确提取（绝不从像素里猜数字）：图表从 PDF 里裁原图，关键数字逐条核对，没有来源的数字不上页面。只有一个主题时，它先联网调研最新信息，再动手。
...
slide-maker 在问你任何问题之前先检查，**已经是最新就完全静默**。有新版时它不会自己更新，也不会只是提一嘴——它会在最开头问你一次：

- **yes** —— 先更新，再用新版本开工。
- **no** —— 用现装的版本。做到一半时这是对的：一份 deck 前后由两个版本造出来，比整份用旧版更糟。
- **other** —— 你有本地改动。它会把「哪些是你的、哪些是新的」摆给你看，自己不做任何决定。

远端查询缓存 24 小时，但**你自己有没有未提交的改动是每次重新读的** —— 一天前的「你的工作区是干净的」，正是会让人的改动被覆盖的那个读数。断网时保持安静而不是碍事。
...
- **风格由你先挑，不用开口要。** 当它从零给你设计（你自己没有模板）时，会**渲染四个风格方向**成一个浏览器链接，等你选完再动手。四个里**至少有一个是为你这个主题原创的 register**，而不是从预设里挑的；并且有一道机械检查会挡掉「四种同一件事的配色」——所以那是真的选择，不是走过场。
