...
这张表说的是**格式**。现在已经有几个新工具也能输出原生可编辑 PPTX，所以「可编辑」本身不再是区分点。真正的区别在于：读你的真实材料、拒绝编造；交付前先过独立评审；每一页围绕你的内容编排，而不是把文字倒进模板。
...
| `review:` | 跑什么 | 实测量级 |
| --- | --- | --- |
| `fast` | 1 名通才评审、1 轮、前 5 条关键声明查证 —— 需要你主动点名才会用 | ~6 个 subagent · ~25 万 token |
| `standard` | 2 名单镜头评审（内容 + 设计）、2 轮、前 10 条 —— 组会、周报的默认档 | ~12 · ~60 万 |
| `thorough` | 多评审面板 + 仲裁复核、3 轮、全部声明 —— 答辩、路演的默认档 | ~32 · ~200 万 |

<sub>实测自一份 12 页、来源全部来自联网调研的 deck，2026 年 7 月，请当量级看，不是保证。什么都不说，档位由你的用途决定。</sub>
