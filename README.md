...
slide-maker tells you when it has fallen behind. At the start of a build it prints **one line** if a newer version exists, and **nothing at all** when you are current. It never updates itself and never blocks: the check is cached for 24 hours, and if you are offline it stays quiet rather than getting in the way.
